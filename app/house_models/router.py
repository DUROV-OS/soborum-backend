...
require_house_models = require_module(AccessModule.HOUSE_MODELS)


@app.get("/catalog", response_model=HouseModelCatalogOut)
def catalog(db: Session = Depends(get_db), _: User = Depends(require_house_models)):
    return house_models_service.get_catalog(db)


@app.get("/catalog/{key}", response_model=HouseModelDetailOut)
def catalog_detail(key: str, db: Session = Depends(get_db), _: User = Depends(require_house_models)):
