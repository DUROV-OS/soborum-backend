...
def list_productions(
    ...
    production_ids = [p.id for p, _, _ in rows]
    completed_by_id = production_service.completed_flags_by_production(db, production_ids)
    criticality_by_id = production_service.criticality_by_production(db, production_ids, completed_by_id)
    return [ProductionListOut(id=p.id, cycle_id=p.cycle_id, house_index=p.house_index, name=p.name,
                              ...
                              is_completed=completed_by_id[p.id], criticality=criticality_by_id[p.id])
